...
A 诚实性（除思考卫生外）、B TurnActivity、C LiveStore、D 打磨 — **全部待做**。
...
| **T0.6** | 提交工作树改动 / 开 PR | 尚未 commit | **todo** |
...
| ID | 任务 | 改造方案 | 触点 | 验收 | 状态 |
|----|------|----------|------|------|------|
| **A1** | 失败工具主对话可见 | `isToolStepMessage` 且 `isError`/failed → **不再 `return null`**；渲染轻量 `FailedToolRow`（红点 + 标题 + 可选 detail）；成功 tool_step 仍隐藏 | `ConversationThread` · `AgentActivity` · i18n | 失败刷新后仍见；8 次成功 Read 不堆行 | **todo** |
| **A2** | 统一回合结束芯片 | 收敛 `turn_cancelled` / `stream_stall` / 部分 turn_error → `EndOfTurnChip`；`reason`: `user_stop` \| `agent_exit` \| `stall` \| `permission_denied` \| `error` | `session.ts` · `App.tsx` 事件 · `EndOfTurnChip.tsx` | Stop/stall/错误各一稳定文案、无双 banner | **todo** |
| **A3** | Stop 门闩 | 用户 Stop 后 **T=2000ms** Host 未离开 busy → 前端 force 可发送 + EndOfTurn；核对 Host interrupt/kill 必达 | stop 路径 · 可选 `session://force_idle` | 人为挂起 interrupt 不锁死 composer；门闩单测 | **todo** |
| **A4** | tool-only / 空助手回合 | 回合结束无正文但有 tool 活动 → 至少落 TurnActivity 或「完成 N 步」占位（不写假散文） | 回合完成路径 · 与 B2 可同 PR | 仅工具无正文的回合重开仍可见活动 | **todo** |
| **A5** | 更新 wiki | `session-continuity.md`：失败例外 + EndOfTurn + 思考标签规则 | docs/llm-wiki | 与实现一致 | **todo** |
...
| ID | 任务 | 改造方案 | 触点 | 验收 | 状态 |
|----|------|----------|------|------|------|
| **B1** | `TurnActivity` view-model | `src/lib/turnActivity.ts`：按 last user 之后收集 tools；字段 id/name/kind/status/summary/isError；`modifiedPaths` 供 Diff；从 `sessionTasks` 抽取共享 collector | `turnActivity.ts` · 重构 `sessionTasks.ts` | 单测：归组阈值、errorCount、同源 | **todo** |
| **B2** | `TurnActivityBlock` UI | 挂本回合末 assistant 下；默认折叠，**有 error 默认展开**；标题 `本回合活动 · {n} 步` / `· {e} 失败`；展开体工具列表；≥3 连续读类 → `Gathering context (N)`；**直播不替代** live 行（推荐回合结束后固化） | `lobe-chat/TurnActivityBlock.tsx` · `ConversationThread` | 多工具回合可见；成功不刷屏 | **todo** |
| **B3** | 轻量 Tool 注册表 | `toolDisplay.ts`：kind → icon / shortLabel / summarize(path,detail,title)；覆盖 bash/read/edit/search/subagent/fallback | 摘要 + Tasks 面板共用 | 侧栏与摘要同标题逻辑 | **todo** |
| **B4** | 思考摘要（产品完善） | 已实现 extract + 去编号；本项 = 回归与文案打磨（hover 时长、超长 ellipsis） | Thinking · CSS | 无「思考 N」；中英摘要可读 | **partial→收尾** |
| **B5** | 对接 AgentTasksPanel | Panel 改为消费 `collectSessionTasks` / turnActivity 同一导出；删重复解析 | `AgentTasksPanel.tsx` | 改一处两边同步 | **todo** |
| **B6** | wiki 活动规则 | session-continuity 写清直播/历史/失败/摘要四条 | docs | — | **todo** |
...
| ID | 任务 | 改造方案 | 触点 | 验收 | 状态 |
|----|------|----------|------|------|------|
| **C1** | `SessionLiveStore` | `Map<sessionId, Snapshot>`：state / streamingMessageId / liveTool / startedAt / terminalReason；事件 adapter 先写 store 再 render；切会话不 clear 其他 session | hooks 或 thin store · App 事件入口 | A 流式切 B 再回 A live 正确 | **todo** |
| **C2** | pure 对账 `sessionPhase.ts` | 输入 Host SessionState + 本地门闩 + 最后事件时间 → composer 可否发 / quiet thinking / force idle；打开会话、visibility、Stop 超时调用 | lib + 单测 | stuck busy 用例绿 | **todo** |
| **C3** | 侧栏 streaming 指示 | 非当前会话 `streaming \| awaiting_permission` 显示 busy/权限点 | 会话列表 · i18n · a11y | 他会话在跑可见 | **todo** |
| **C4** | 从 App 抽事件族 | 按 PR 抽 tool/compact/marker/error/stall → store；**禁止**一次搬 10k 行 | App.tsx 减负 | 可量化行数下降 | **todo** |
...
| ID | 任务 | 改造方案 | 优先级 | 状态 |
|----|------|----------|--------|------|
| **D1** | 回合 Diff 条 | TurnActivity 下「修改了 N 个文件」→ 打开 ResourceViewer Changes | 高 | **todo** |
| **D2** | 长任务 detail 尾部 | 摘要展开显示 `toolDetail` 最后 N 行（非全量 log） | 中 | **todo** |
| **D3** | 纯文本 soft buffer | 首段约 40 词或 2.5s；代码围栏旁路 | 中 | **todo** |
| **D4** | 双档静默文案 | 首 token 前/后不同 stall 提示 | 中 | **todo** |
| **D5** | 长会话虚拟列表 | 接现有 VirtualList，测 stick | 中（有长会话数据再上） | **todo** |
| **D6** | bootstrap normalizer | journal 回灌时工具摘要用 XML 风格标签，防 few-shot | 中 | **todo** |
| **D7** | 发送前轻量 checkpoint | 仅阻塞态（无 CLI/无项目等）inline，非 modal；对齐 ErrorDeck | 中 | **todo** |
...
| 5 | T0.5 | 0 | 相关单测 | done |
| 6 | T0.6 | 0 | 提交 / PR | **todo** |
| 7 | A1 | A | 失败 tool 主对话可见 | **todo** |
| 8 | A2 | A | EndOfTurnChip 统一终态 | **todo** |
| 9 | A3 | A | Stop 2s 门闩 | **todo** |
| 10 | A4 | A | tool-only 回合可回放 | **todo** |
| 11 | A5 | A | 更新 session-continuity wiki | **todo** |
| 12 | B1 | B | TurnActivity view-model 同源 | **todo** |
| 13 | B2 | B | TurnActivityBlock 折叠 UI | **todo** |
| 14 | B3 | B | toolDisplay 注册表 | **todo** |
| 15 | B4 | B | 思考摘要体验收尾 | partial |
| 16 | B5 | B | AgentTasksPanel 同源 | **todo** |
| 17 | B6 | B | wiki 活动规则 | **todo** |
| 18 | C1 | C | SessionLiveStore | **todo** |
| 19 | C2 | C | sessionPhase 对账 | **todo** |
| 20 | C3 | C | 侧栏他会话 busy | **todo** |
| 21 | C4 | C | App 事件族抽离 | **todo** |
| 22 | D1 | D | 回合 Diff 条 → Changes | **todo** |
| 23 | D2 | D | 工具 detail 尾输出 | **todo** |
| 24 | D3 | D | 纯文本 soft buffer | **todo** |
| 25 | D4 | D | 双档 stall 文案 | **todo** |
| 26 | D5 | D | 虚拟列表 | **todo** |
| 27 | D6 | D | bootstrap normalizer | **todo** |
| 28 | D7 | D | 发送前 checkpoint | **todo** |

**统计**：done 5 · partial 1 · todo 22（含 T0.6 提交）
