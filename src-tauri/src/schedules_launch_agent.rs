...
fn limits_text(app_path: &Path, helper: &Path) -> String {
    format!(
        "Grok App — schedules LaunchAgent helper\n\
         ======================================\n\
         \n\
         ...
         \n\
         What it does NOT do:\n\
         - It does not run scheduled tasks by itself.\n\
         - automation_runner only ticks while the Grok App process is alive\n\
           (main window or tray). A normal Quit pauses schedules.\n\
         \n\
         App path: {app}\n\
         Helper dir: {dir}\n\
         Label: {label}\n\
         \n\
         Disable from Grok → Scheduled tasks, or:\n\
...
/// Write helper script + plist + LIMITS under app data (no launchctl).
