...
const en = {
  ...
  "reliability.timeline.clearConfirmMessage":
    "This removes the local stall history ring from this device. Live stall signals are unaffected.",
  ...
  "session.exportImageHint":
    "Preview below, then copy or save. Enable Smart summary poster for an auto-styled condensed card. Footer always says “Generated with Grok App”. Optional custom logo is configured in Settings → Appearance → Interface.",
  "session.exportImageSmart": "Smart summary poster",
  "session.exportImageSmartDesc":
    "Condense the chat into a branded poster. Color and layout are derived from content structure (not fixed topic buckets).",
  "session.exportImageStyleAuto": "Layout: {style}",
  ...
  "ext.hooks.try.title": "Try / override",
  "ext.hooks.try.desc":
    "Paste sample hook stdin JSON to validate shape and record a synthetic dry-run in Recent activity. This does not execute shell hooks or call the host runner.",
  ...
  "ext.hooks.try.stdin": "Sample stdin JSON",
...
const zh: Record<MessageKey, string> = {
  ...
  "session.exportImageHint":
    "下方预览分享卡片，确认后复制或选择保存位置。开启「智能总结海报」可按内容自动换风格并压缩要点。页脚固定 “Generated with Grok App”。自定义 Logo 可在 设置 → 外观 → 界面 上传。",
  "session.exportImageSmart": "智能总结海报",
  "session.exportImageSmartDesc":
    "将对话压缩为品牌海报；配色与版式依内容结构自动生成（非固定主题分类）。",
  ...
  "ext.hooks.try.title": "试跑 / 覆盖",
  "ext.hooks.try.desc":
    "粘贴示例 hook stdin JSON 以校验结构，并在「最近活动」中写入一条合成试跑记录。不会执行 shell hook，也不会调用主机 runner。",
  "ext.hooks.try.hookName": "Hook 名称",
  "ext.hooks.try.hookNamePlaceholder": "可选 — 选择或输入文件名",
  "ext.hooks.try.eventType": "事件类型",
  "ext.hooks.try.stdin": "示例 stdin JSON",
