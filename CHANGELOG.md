...
- **Event routing never fails silently**: ACP events whose process matches no live/background session are recovered (a still-streaming parked agent is moved back to `background`) or logged at `warn`, instead of `return`-ing into the void.
...
- **事件路由不再静默失败**：进程对不上任何 live/background 会话的 ACP 事件，要么被救回（仍在输出却被 park 的 agent 移回 `background`），要么打 `warn` 日志，不再直接 `return` 吞掉。
